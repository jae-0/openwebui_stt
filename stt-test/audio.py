...
@router.get("/speech/{uuid}/download",
            response_class=JSONResponse)
def download_transcript(uuid: str):
    """전사본을 TXT 파일로 저장 후 다운로드 URL 돌려줌."""
    src = SPEECH_CACHE_DIR / f"{uuid}.json"
    if not src.exists():
        raise HTTPException(404, "transcript not found")

    text = json.load(src.open("r", encoding="utf-8"))["text"]

    # 파일명 중복 방지를 위해 uuid 유지
    dst = DOWNLOAD_DIR / f"{uuid}.txt"
    dst.write_text(text, encoding="utf-8")

    url = f"/files/{uuid}.txt"         # StaticFiles 경로
